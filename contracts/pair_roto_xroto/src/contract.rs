...
/// Implementation of the bonded pair template. Performs ROTO-xROTO swap operations.
impl<'a> PairBonded<'a> for Contract<'a> {
    const CONTRACT_NAME: &'a str = "rotosports-pair-roto-xroto";
    ...
    fn swap(
        ...
        if offer_asset.info.equal(&AssetInfo::Token {
            contract_addr: params.roto_addr.clone(),
        }) {
            messages.push(CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: params.roto_addr.to_string(),
            ...
            messages.push(CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: params.xroto_addr.to_string(),
                msg: to_binary(&Cw20ExecuteMsg::Send {
                    contract: params.staking_addr.to_string(),
                    amount: offer_asset.amount,
    ...
    fn query_simulation(
        ...
        let total_deposit =
            query_token_balance(&deps.querier, &params.roto_addr, &params.staking_addr)?;
        let total_shares = query_supply(&deps.querier, &params.xroto_addr)?;

        let return_amount = if offer_asset.info.equal(&AssetInfo::Token {
            contract_addr: params.roto_addr,
        }) {
    ...
    /// Reverse simulation swap using Rotosports Staking contract.
