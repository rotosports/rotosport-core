# Rotosports Constant Product Pair

The constant product pool uses the widely known xy=k formula. More details around how the pool functions can be found [here](https://docs.rotosports.fi/rotosports/rotosports/roto-pools/constant-product-pools).
...
As an example, let's say the global ratio between two tokens x:y is 10:2 (i.e. 1 x = 0.2 y), but the current ratio between the tokens in an Rotosports pair is 5:2 (1 x = 0.4 y). Let's also say that someone may decide to LP in the x:y Rotosports pool at the current 5:2 ratio. As the Rotosports pool gets arbitraged to the global ratio, the amount of x & y tokens that the LP can withdraw changes because the total amounts of x & y tokens in the pool also change.
