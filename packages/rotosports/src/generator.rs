...
pub struct InstantiateMsg {
    ...
    /// Amount of ROTO distributed per block among all pairs
    pub tokens_per_block: Uint128,
    /// Start block for distributing ROTO
    pub start_block: Uint64,
    /// The ROTO vesting contract that drips ROTO rewards
...
#[cw_serde]
pub enum ExecuteMsg {
    /// Update the address of the ROTO vesting contract
    /// ## Executor
    ...
    /// Update the given pool's ROTO allocation slice
    /// ## Executor
    /// Only the owner or generator controller can execute this.
    UpdatePool {
    ...
    /// Set a new amount of ROTO to distribute per block
    /// ## Executor
    /// Only the owner can execute this.
    SetTokensPerBlock {
        /// The new amount of ROTO to distro per block
        amount: Uint128,
    },
...
pub enum ExecuteOnReply {
    ...
    /// Sets a new amount of ROTO to distribute per block between all active generators
    SetTokensPerBlock {
        /// The new amount of ROTO to distribute per block
        amount: Uint128,
    },
...
pub enum QueryMsg {
    ...
    /// the total pending amount of ROTO and proxy rewards claimable by generator stakers (for that LP token)
    #[returns(PoolInfoResponse)]
    PoolInfo { lp_token: String },
    /// SimulateFutureReward returns the amount of ROTO that will be distributed until a future block and for a specific generator
    #[returns(Uint128)]
...
pub struct UserInfo {
    ...
    pub amount: Uint128,
    /// The amount of ROTO rewards a user already received or is not eligible for; used for proper reward calculation
...
pub struct PoolInfoResponse {
    /// The slice of ROTO that this pool's generator gets per block
    pub alloc_point: Uint128,
    /// Amount of ROTO tokens being distributed per block to this LP pool
    pub roto_tokens_per_block: Uint128,
    ...
    /// Total amount of ROTO rewards already accumulated per LP token staked
    pub global_reward_index: Decimal,
    /// Pending amount of total ROTO rewards which are claimable by stakers right now
    pub pending_roto_rewards: Uint128,
